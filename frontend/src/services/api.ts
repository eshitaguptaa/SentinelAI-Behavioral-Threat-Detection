...
export function buildDemoFeatureVectors(
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    vectors.push({
      ...
      country_change_count: tier >= 3 ? 2 + (i % 2) : i % 2 === 0 ? 0 : 1,
      location_change_count: 1 + (i % 5),
      unique_device_count: 1 + (i % 4),
      unique_location_count: 1 + (i % 3),
      resource_entropy: tier >= 3 ? 2.4 : 0.4 + (i % 5) * 0.2,
      device_entropy: tier === 4 ? 1.6 : 0.2 + (i % 4) * 0.15,
      after_hours_event_count: tier >= 3 ? 12 + (i % 8) : i % 4,
      download_size_mb_sum: tier === 4 ? 180 : tier === 3 ? 70 : 5 + i,
      mass_download_event_count: tier === 4 ? 2 : 0,
      vpn_usage_ratio: 0.1 + (i % 6) * 0.08,
      burst_max_5min: tier >= 3 ? 22 + (i % 10) : 4 + (i % 6),
      active_duration_hours: 7 + (i % 8),
      file_access_ratio: 0.15 + (i % 5) * 0.08,
